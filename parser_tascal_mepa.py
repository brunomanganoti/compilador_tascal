...
# Precedência igual a do seu amigo
precedence = (
    ('left', 'OR'),
    ('left', 'AND'),
...
def p_programa(p):
    ...
    # p[2] é o nome, p[4] é o bloco
    p[0] = ast.Programa(id=p[2], declaracoes=p[4]['decls'], bloco=p[4]['cmds'])

def p_bloco(p):
    """bloco : declaracoes comando_composto"""
    # Retorna um dicionário auxiliar para separar declarações de comandos
    # O comando_composto retorna um objeto Bloco, pegamos os comandos dele
    p[0] = {
        'decls': p[1],
        'cmds': p[2] # p[2] já é um ast.Bloco ou ast.ComandoComposto
    }

# --- Declarações (Lógica do seu amigo adaptada) ---

def p_declaracoes(p):
    """declaracoes : VAR declaracao_variaveis
                   | empty"""
    if len(p) == 3:
        p[0] = p[2] # Retorna lista de ast.Declaracao
    else:
        p[0] = []
...
def p_declaracao_variaveis(p):
    ...
    vars_nodes = [ast.Var(nome=n) for n in p[1]]
    decl = ast.Declaracao(ids=vars_nodes, tipo=p[3])
    
    if len(p) == 6:
        p[0] = [decl] + p[5] # Junta com as próximas
    else:
        p[0] = [decl]
...
# --- Comandos (A gramática recursiva do seu amigo) ---
...
def p_lista_comandos(p):
    """lista_comandos : comando
                      ...
                      | comando PV"""
    # Essa regra lida perfeitamente com ; opcionais ou obrigatórios
    cmd = p[1]
    cmds = []
    
    # Se comando não for vazio/none
    if cmd:
        cmds.append(cmd)
    
    if len(p) == 4:
        # comando ; lista_restante
        cmds += p[3]
...
def p_expressao_or(p):
    """expressao : expressao OR expressao_and
                 | expressao_and"""
    if len(p) == 4:
        p[0] = ast.BinOp(esq=p[1], op='or', dir=p[3])
    else:
        p[0] = p[1]
...
def p_expressao_and(p):
    """expressao_and : expressao_and AND expressao_rel
                     | expressao_rel"""
    if len(p) == 4:
        p[0] = ast.BinOp(esq=p[1], op='and', dir=p[3])
    else:
        p[0] = p[1]
...
def p_expressao_rel(p):
    """expressao_rel : soma relacao soma
                     | soma"""
    if len(p) == 4:
        p[0] = ast.BinOp(esq=p[1], op=p[2], dir=p[3])
    else:
        p[0] = p[1]
...
def p_soma(p):
    """soma : soma MAIS termo
            ...
            | termo"""
    if len(p) == 4:
        op = '+' if p[2] == '+' else '-' # Normaliza para + ou -
        p[0] = ast.BinOp(esq=p[1], op=op, dir=p[3])
    else:
        p[0] = p[1]
...
def p_fator(p):
    """fator : ID
             ...
             | NOT fator
             | MENOS fator"""
    # Identifica qual regra foi usada pelo tipo do token ou tamanho
    if len(p) == 2:
        token_type = p.slice[1].type
        if token_type == 'ID':
            p[0] = ast.Var(nome=p[1])
        elif token_type == 'NUM':
            p[0] = ast.Num(valor=p[1])
        elif token_type in ['TRUE', 'FALSE']:
            val = True if p[1].lower() == 'true' else False
            p[0] = ast.Bool(valor=val)
    elif len(p) == 4: # ( expressao )
        p[0] = p[2]
    elif len(p) == 3: # NOT fator ou - fator
        op = p[1]
        if op == 'not': op = 'not'
        else: op = '-'
        p[0] = ast.UnOp(op=op, expr=p[2])
